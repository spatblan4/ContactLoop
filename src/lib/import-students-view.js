...
export function renderImportMapping(state) {
  ...
  return `${renderHeader(state)}<section class="import-panel"><div class="import-panel-heading"><div><h2>Match your columns</h2><p>Review the suggested mappings before we prepare your import.</p></div><span class="import-count">${state.headers.length} source columns</span></div><div class="mapping-list">${mappings || '<div class="empty-state">No columns found in this file.</div>'}</div><div class="import-actions"><button class="button-secondary" data-import-back>Back</button><button class="button-primary" data-import-continue>Continue to review <span>→</span></button></div></section>`;
}
...
export function renderImportReview(state) {
  ...
  return `${renderHeader(state)}<section class="import-panel import-review-panel"><div class="import-panel-heading"><div><h2>Ready to import</h2><p>Check the rows below. We will not write records until you confirm.</p></div><div class="import-summary"><strong>${stats.students} student${stats.students === 1 ? '' : 's'}</strong><strong>${stats.guardians} guardian${stats.guardians === 1 ? '' : 's'}</strong><strong>${stats.needsReview} rows need review</strong></div></div><div class="import-table-wrap"><table class="import-table"><thead><tr><th>Student</th><th>Guardian</th><th>Relationship</th><th>Phone</th><th>Email</th><th>Status</th></tr></thead><tbody>${rows || '<tr><td colspan="6" class="empty-state">No rows found.</td></tr>'}</tbody></table></div><div class="import-actions"><button class="button-secondary" data-import-back>Back</button><button class="button-primary" data-import-submit ${stats.ready === 0 && stats.duplicates === 0 ? 'disabled' : ''}>Import ready records <span>→</span></button></div></section>`;
}
