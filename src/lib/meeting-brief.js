...
export function buildMeetingBrief({ events = [], students = [], followUps = [], studentId, range = { preset: 'this-month' }, now = new Date(), teacherName = 'Bella Rivera' }) {
  ...
  const openFollowUps = followUps.filter(followUp => (followUp.status ?? 'open') === 'open' && (!studentId || (followUp.studentId ?? followUp.student_id) === studentId));
  const notes = rangeEvents.map(event => event.note).filter(Boolean);
  ...
  return {
    ...
    openFollowUps,
    insights,
    detailedHistory: [...rangeEvents].sort((left, right) => new Date(right.callTime) - new Date(left.callTime)).map(event => ({
      ...event,
