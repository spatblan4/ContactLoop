...
def create_follow_up(
    ...
    follow_up = FollowUpDAO(db).create(payload.model_dump(), actor_id=user_id)
    db.commit()
    return follow_up


@router.get("/{follow_up_id}", response_model=FollowUpRead)
def get_follow_up(follow_up_id: uuid.UUID, db: Session = Depends(get_db)):
    return FollowUpDAO(db).require(follow_up_id)
...
def update_follow_up(
    ...
    if data.get("status") == "completed":
        follow_up = FollowUpDAO(db).get(follow_up_id)
        if follow_up is not None and follow_up.completed_at is None:
            data["completed_at"] = datetime.now(timezone.utc)
    follow_up = FollowUpDAO(db).update(follow_up_id, data, actor_id=user_id)
    db.commit()
    return follow_up
...
def delete_follow_up(
    ...
    FollowUpDAO(db).soft_delete(follow_up_id, actor_id=user_id)
    db.commit()
